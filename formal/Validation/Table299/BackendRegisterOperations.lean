import Validation.Table299.PhysicalRegisters
import Validation.Table299.BackendIncrement
import Validation.Table299.BackendDecrement
import Validation.Table299.BackendInitialize
namespace RiemannMachineVerification

/-- [proof-guide] Incrementing a logical register has the required effect on the complete tape encoding.
Proof: Combines `Nat.le_pred_of_lt` with the displayed local equations.
...
theorem backend_increment_encoded (capacity : ℕ) (values : RegisterIndex → ℕ)
    ...
    ZExec machine381
      (zipperOf (backendConfig (some (backendIncrement r)) 12 pc (encodedStorage capacity values)))
...
/-- [proof-guide] Decrementing a logical register has the required value and branch effects on the complete tape encoding.
Proof: Combines `Nat.le_pred_of_lt` with the displayed local equations.
Role: Supports the separate acceptance checks; this module is outside the 278-state headline dependency chain. -/
theorem backend_decrement_encoded (capacity : ℕ) (values : RegisterIndex → ℕ)
    (r : RegisterIndex) (hr : (registerSlot r).val < capacity) (pc : ProgramCounter) :
    ZExec machine381
      (zipperOf (backendConfig (some (backendDecrement r)) 12 pc (encodedStorage capacity values)))
