# Finite certificates for the 299-state target

Counter and dispatcher files cover all 2048 instruction-counter values in
64-address chunks, with both counter updates. Macro case files cover the
shortened machine's states in 20-state chunks. `Invariant0` through `Invariant3`
prove successive inductive read masks using explicit Boolean certificates.

Aggregate theorems in the parent folder quantify over all cases. Chunking bounds
Lean elaboration jobs; the repeated equations concern distinct concrete indices.
Each local proof guide identifies its case and explains its certificate role.
These certificates are checked by the separate `Validation` build.
