import RiemannMachineVerification.Generated.RegisterRefinement.TransferCoverage
namespace RiemannMachineVerification.Implementation

/-- [proof-guide] Every transfer-level instruction expands to a positive primitive execution with the same result.
Proof: Combines `hsite` with the displayed local equations.
