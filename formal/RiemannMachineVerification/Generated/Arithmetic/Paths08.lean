...
/-- [proof-guide] Checks the displayed arithmetic register path at address 256, including its branch guard and final register values.
...
/-- [proof-guide] Checks the displayed arithmetic register path at address 512, including its branch guard and final register values.
Proof: Combines `registerConfiguration_ext` with the displayed local equations.
Role: Connects the arithmetic computation and its search to the exact Counterexample predicate. -/
...
/-- [proof-guide] The first harmonic block reaches the displayed exact numerator and denominator values.
Proof: Combines `registerConfiguration_ext` with the displayed local equations.
Role: Connects the arithmetic computation and its search to the exact Counterexample predicate. -/
