...
/-- [proof-guide] A local tape step assembles to the corresponding infinite-tape step.
...
/-- [proof-guide] One local step moves the head by at most one cell.
Proof: Rewrites the defining equations and the cited arithmetic or execution facts.
Role: Supplies the simulation and execution facts used to compose the headline equivalence. -/
...
/-- [proof-guide] After a bounded number of local steps, the head stays within the resulting distance bound.
...
/-- [proof-guide] A bounded local execution assembles to the corresponding infinite-tape execution.
Proof: Induction on `t`, using the displayed step equations and auxiliary lemmas.
Role: Supplies the simulation and execution facts used to compose the headline equivalence. -/
...
/-- [proof-guide] Extracting and reassembling a local view recovers the original configuration.
Proof: Rewrites the defining equations and the cited arithmetic or execution facts.
Role: Supplies the simulation and execution facts used to compose the headline equivalence. -/
...
/-- [proof-guide] Assembling a local configuration commutes with renaming states.
