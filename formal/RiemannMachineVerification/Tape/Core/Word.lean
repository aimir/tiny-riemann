...
/-- [proof-guide] Carrying through an appended tape word equals appending its carried pieces.
Proof: Induction on `w`, using the displayed step equations and auxiliary lemmas.
...
/-- [proof-guide] Encoding an appended tape word concatenates its bit encodings.
...
/-- [proof-guide] The reverse-carried word has the specified reversed bit encoding.
Proof: Induction on `w`, using the displayed step equations and auxiliary lemmas.
Role: Connects operations on the binary tape to unbounded natural-valued register execution. -/
...
/-- [proof-guide] The carried-word encoding has the required prefix form.
...
/-- [proof-guide] Reversing the carried representation recovers the stated original word representation.
Proof: Rewrites the defining equations and the cited arithmetic or execution facts.
Role: Connects operations on the binary tape to unbounded natural-valued register execution. -/
...
/-- [proof-guide] Reversal converts the word prefix to the stated suffix representation.
Proof: Rewrites the defining equations and the cited arithmetic or execution facts.
Role: Connects operations on the binary tape to unbounded natural-valued register execution. -/
...
/-- [proof-guide] A word of unary marks encodes as a list of true bits.
