...
/-- [proof-guide] After the specified write, the return routine reaches its encoded return position.
Proof: Combines `ZExec.prepend`, `ZExec.single` with the displayed local equations.
...
/-- [proof-guide] After crossing a separator, the return routine reaches its encoded return position.
Proof: Combines `ZExec.single` with the displayed local equations.
Role: Connects operations on the binary tape to unbounded natural-valued register execution. -/
