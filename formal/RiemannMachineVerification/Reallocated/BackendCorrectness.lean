import RiemannMachineVerification.Reallocated.BackendRegisterOperations
import RiemannMachineVerification.Reallocated.RegisterValidity
import RiemannMachineVerification.Reallocated.DispatcherCorrectness

namespace RiemannMachineVerification.Reallocated

def RegisterRep (z : Zipper 381) (s : RegisterConfiguration) : Prop :=
  match s.pc with
  | none => z.state = none
  | some pc => z = encodedRegisterZipper pc (encodedStorage s.capacity s.values)

def RegisterRelation (z : Zipper 381) (s : RegisterConfiguration) : Prop :=
  RegisterValid s ∧ RegisterRep z s

theorem encodedRegister_initial :
    encodedRegisterZipper ⟨0, by decide⟩ [] =
      ⟨some machine381.entry, fun _ => false, fun _ => false⟩ := by
  apply zipper_ext
  · rfl
  · funext n
...
theorem registerRelation_initial :
    RegisterRelation ⟨some machine381.entry, fun _ => false, fun _ => false⟩
      initialRegisterConfiguration := by
  refine ⟨initialRegisterValid, ?_⟩
  exact encodedRegister_initial.symm

theorem registerRelation_halts (z : Zipper 381) (s : RegisterConfiguration)
...
theorem backend_primitive_advance (s : RegisterConfiguration) (pc : ProgramCounter)
  ...
  | jump next =>
    ...
    · simpa only [dispatcherTarget, hi, encodedRegisterZipper, backendConfig, initialPrefix] using
        ZExec.refl machine381 (encodedRegisterZipper next (encodedStorage s.capacity s.values))
    · simp only [RegisterRep, registerStep, hp, hi]
  | halt =>
    apply finish (zipperOf (backendConfig none 12 pc (encodedStorage s.capacity s.values)))
    · simpa only [dispatcherTarget, hi, backendConfig] using
        ZExec.refl machine381 (zipperOf (backendConfig none 12 pc (encodedStorage s.capacity s.values)))
    · simp only [RegisterRep, registerStep, hp, hi]
      rfl
  | transfer source targets next => simpa only [hi] using hshape

theorem registerRelation_advance (z : Zipper 381) (s : RegisterConfiguration)
    (h : RegisterRelation z s) :
    ∃ t, 0 < t ∧ RegisterRelation (trajectory (zipperStep machine381) z t)
      (registerStep primitiveProgram s) := by
  obtain ⟨hv, hr⟩ := h
  cases hp : s.pc with
  | none =>
...
theorem machine381_iff_primitive : HaltsBlank machine381 ↔ RegisterHalts primitiveProgram := by
  rw [halts_iff_zipper]
  exact eventually_iff_of_simulation (zipperStep machine381) (registerStep primitiveProgram)
    (fun z => z.state = none) (fun s => s.pc = none)
    ⟨some machine381.entry, fun _ => false, fun _ => false⟩ initialRegisterConfiguration
