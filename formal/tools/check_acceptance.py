...
from layout import FORMAL, MODULES, path
...
for name, digest in PINS.items():
    source = path(MODULES['RiemannMachineVerification.' + name])
    if hashlib.sha256(source.read_bytes()).hexdigest() != digest:
        raise SystemExit(f'Acceptance failed: approved definition changed: {source}')
...
subprocess.run([sys.executable, 'tools/build.py', 'Validation'], cwd=ROOT, check=True)
...
''')
    result = subprocess.run(lean_command(probe), cwd=ROOT,
